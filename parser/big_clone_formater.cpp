...
string Big_Clone_Formater::build_path_formated_string(Path path){
	...
	auto [start_line,end_line] = function.get_scope_function_in_file();
...
auto Big_Clone_Formater::process_similar_path_pair(Path path1, Path path2){
...
Big_Clone_Formater::Big_Clone_Formater(Similarity_Table *similarity_table){
	auto similar_paths = similarity_table->get_all_similar_path_pairs_sorted_by_similarity();
	for(auto [path1,path2] : similar_paths){
